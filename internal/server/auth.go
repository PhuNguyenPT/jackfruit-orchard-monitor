...
func (s *Server) registerPageHandler(c *gin.Context) {
	...
	if err := views.RegisterPage(getUserName(c), getLangStr(c), next, s.siteConfig(c)).Render(c.Request.Context(), c.Writer); err != nil {
...
func (s *Server) loginPageHandler(c *gin.Context) {
	...
	c.Status(http.StatusOK)
	c.Header("Content-Type", "text/html; charset=utf-8")
	if err := views.LoginPage(getUserName(c), getLangStr(c), next, s.siteConfig(c)).Render(c.Request.Context(), c.Writer); err != nil {
