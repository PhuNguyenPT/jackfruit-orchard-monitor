...
7.14 PhuNguyenPT 10.07.26 Repurposed Device Holder 1 / PCB1 from the solar PCB controller
    (M2.5, 45.0x40.0x10.1mm, 32.5x37.5mm hole spacing) to the DC-DC Buck LM2596 3A LED
    (ZX-052 V2.0) module (M3, 66x36mm PCB, 61.2x31.2mm hole spacing, 3.2mm holes assumed).
    Moved the PCB1_* reference-block variables above Offset_X_1/Offset_Y_1 in file order,
    since Offset_Y_1 now reads PCB1_Length directly (breadboard-gap anchoring) and OpenSCAD
    evaluates top-level assignments in file order, not as a dependency graph - leaving them
    below silently resolved PCB1_Length to undef and cascaded into every DeviceHolder1 boss
...
DeviceHolder1_ScrewSize    = 3;
// ISO metric coarse thread pitch in mm for the size above: m2=0.4 // m2.5=0.45 // m3=0.5 // m4=0.7 // m5=0.8
DeviceHolder1_ThreadPitch  = 0.5;

// The diameter of the screw cylinder (standoff width)
ScrewCylinderDiameter1    = 7.0;

// The height of the screw cylinders (standoff height)
ScrewCylinderHeight1      = 6.0;

// Distance between the holes in X direction
DeviceHolder_X_Distance1  = 61.2;

// Distance between the holes in Y direction
DeviceHolder_y_Distance1  = 31.2;
...
GapToBreadboardY           = 5.0;   // [0:0.25:50]

// UNUSED as of the buck-converter repurpose: Offset_Y_1 is now derived from GapToBreadboardY
// (breadboard-relative anchoring), not from this wall-clearance value. Left in place only so
...
// Port 1: free X offset (mm). Nudges the port in/out of its wall - independent of Wall/Y/Z. 0 = default
EthernetPort1_OffsetX      = 0;
// Port 1: free Y offset (mm). Position along the wall (was "Position") - independent of Wall/X/Z. 0 = centered
EthernetPort1_OffsetY      = -3.5;
// Port 1: free Z offset (mm). Vertical position, relative to the default (centered in the top piece, above the breadboard) - independent of Wall/X/Y. 0 = default
EthernetPort1_OffsetZ      = 0;
// Port 2: free X offset (mm). Nudges the port in/out of its wall - independent of Wall/Y/Z. 0 = default
EthernetPort2_OffsetX      = 0;
// Port 2: free Y offset (mm). Position along the wall (was "Position") - independent of Wall/X/Z. 0 = centered
EthernetPort2_OffsetY      = 32.5;
// Port 2: free Z offset (mm). Vertical position, relative to the default (centered in the top piece, above the breadboard) - independent of Wall/X/Y. 0 = default
...
{
    if (ShowEthernetPort1) RJ45Model(EthernetPort1_Wall, EthernetPort1_OffsetX, EthernetPort1_OffsetY, EthernetPort1_OffsetZ);
    if (ShowEthernetPort2) RJ45Model(EthernetPort2_Wall, EthernetPort2_OffsetX, EthernetPort2_OffsetY, EthernetPort2_OffsetZ);
}
    ...
    if (ShowTop)
    {
        difference(){
            union(){
                ...
                if (ShowEthernetPortCollar && ShowEthernetPort1) { color("SlateGray") EthernetPortCollar(EthernetPortWidth,EthernetPortHeight,EthernetPortInsideDepth,EthernetPortCollarWall,EthernetPort1_Wall,EthernetPort1_OffsetX,EthernetPort1_OffsetY,EthernetPort1_OffsetZ); }
                if (ShowEthernetPortCollar && ShowEthernetPort2) { color("SlateGray") EthernetPortCollar(EthernetPortWidth,EthernetPortHeight,EthernetPortInsideDepth,EthernetPortCollarWall,EthernetPort2_Wall,EthernetPort2_OffsetX,EthernetPort2_OffsetY,EthernetPort2_OffsetZ); }
            }
            if (SeeGrooveRidgeScrew)
            {
            ...
            if (ShowEthernetPort1) { EthernetPortCut(EthernetPortWidth,EthernetPortHeight,EthernetPortInsideDepth,EthernetPort1_Wall,EthernetPort1_OffsetX,EthernetPort1_OffsetY,EthernetPort1_OffsetZ); }
            if (ShowEthernetPort2) { EthernetPortCut(EthernetPortWidth,EthernetPortHeight,EthernetPortInsideDepth,EthernetPort2_Wall,EthernetPort2_OffsetX,EthernetPort2_OffsetY,EthernetPort2_OffsetZ); }
        }
    }
}
...
module EthernetPortCut (Width,Height,InsideDepth,Wall,OffsetX,OffsetY,OffsetZ) {
    ...
    // OffsetX/OffsetY/OffsetZ = fully independent, freely settable offsets (mm) in each axis.
    // Wall only selects which face/orientation the port is cut into; OffsetX/Y/Z do NOT depend on
    // each other or on Wall - all three can be changed freely to move the port anywhere in space.
    ...
    ZLocal = (CaseHeight-EthernetPortDefaultZ)-OffsetZ;
    if (Wall=="A") { translate([Caselength/2-TotalDepth/2+OffsetX, OffsetY, ZLocal]) cube([CutDepth,Width,Height],center=true); }
    if (Wall=="C") { translate([-Caselength/2+TotalDepth/2+OffsetX, OffsetY, ZLocal]) cube([CutDepth,Width,Height],center=true); }
    if (Wall=="B") { translate([OffsetX, -CaseWidth/2+TotalDepth/2+OffsetY, ZLocal]) cube([Width,CutDepth,Height],center=true); }
    if (Wall=="D") { translate([OffsetX, CaseWidth/2-TotalDepth/2+OffsetY, ZLocal]) cube([Width,CutDepth,Height],center=true); }
}

module EthernetPortCollar (Width,Height,InsideDepth,CollarWall,Wall,OffsetX,OffsetY,OffsetZ) {
    ...
    // OffsetX/OffsetY/OffsetZ = fully independent, freely settable offsets (mm) in each axis,
    // matching EthernetPortCut()/RJ45Model() so the collar always lines up with the cut and the
    ...
    if (Wall=="A") { translate([Caselength/2-SideWallThickness-InsideDepth/2+OffsetX, OffsetY, NewZCenter]) cube([InsideDepth+0.02,CW,NewHeight],center=true); }
    if (Wall=="C") { translate([-Caselength/2+SideWallThickness+InsideDepth/2+OffsetX, OffsetY, NewZCenter]) cube([InsideDepth+0.02,CW,NewHeight],center=true); }
    if (Wall=="B") { translate([OffsetX, -CaseWidth/2+SideWallThickness+InsideDepth/2+OffsetY, NewZCenter]) cube([CW,InsideDepth+0.02,NewHeight],center=true); }
    if (Wall=="D") { translate([OffsetX, CaseWidth/2-SideWallThickness-InsideDepth/2+OffsetY, NewZCenter]) cube([CW,InsideDepth+0.02,NewHeight],center=true); }
}
...
module RJ45Model (Wall,OffsetX,OffsetY,OffsetZ) {
    // Places RJ45Shape() at a port location using the same Wall/OffsetX/OffsetY/OffsetZ convention
    // as EthernetPortCut()/EthernetPortCollar(), so it automatically lines up with whichever wall
    // and offsets the actual port cutout is configured for. The shape's local X=0 (start of the
    ...
    // OffsetX/OffsetY/OffsetZ are fully independent and freely settable (no dependency on each
    // other or on Wall) - changing them here always keeps the visual model in sync with the cut.
    ZLocal = (CaseHeight-EthernetPortDefaultZ)-OffsetZ;
    if (ShowRJ45Model)
    {
        if (Wall=="C") { translate([-Caselength/2+SideWallThickness+OffsetX, OffsetY, ZLocal]) RJ45Shape(); }
        if (Wall=="A") { translate([Caselength/2-SideWallThickness+OffsetX, OffsetY, ZLocal]) mirror([1,0,0]) RJ45Shape(); }
        if (Wall=="B") { translate([OffsetX, -CaseWidth/2+SideWallThickness+OffsetY, ZLocal]) rotate([0,0,90]) RJ45Shape(); }
        if (Wall=="D") { translate([OffsetX, CaseWidth/2-SideWallThickness+OffsetY, ZLocal]) rotate([0,0,-90]) RJ45Shape(); }
    }
}
...
module PCB1 () {
    // Semi-transparent reference block showing the 66 x 36mm DC-DC Buck LM2596 3A LED
    // (ZX-052 V2.0) module resting on top of Device Holder 1's 4 corner M3 standoffs.
    // Display-only - it is not unioned or subtracted from the printed bottom/top bodies;
    // the real mechanical bosses/holes are the DeviceHolder() calls in BodyBottom() above,
    // which already use the same hole spacing (61.2 x 31.2mm).
    if (ShowPCB1)
    {
        ...
        difference() {
            // NOTE: PCB1_Width (66mm) goes on X, PCB1_Length (36mm) goes on Y - this matches
            // how the real standoffs are laid out (DeviceHolder_X_Distance1=61.2mm spans the
            // 66mm length edge, parallel to the breadboard; DeviceHolder_y_Distance1=31.2mm
            // spans the 36mm width edge). Swapping this would put the box 90 degrees off from
            // the actual screw positions.
