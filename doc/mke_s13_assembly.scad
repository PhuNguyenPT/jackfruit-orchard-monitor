...
module connector_male() {
    ...
    color("White", 0.95) {
        translate([pcb_l - conn_male_d, pcb_w/2 - conn_male_w/2, pcb_t]) {
            difference() {
                union() {
                    // Outer Housing Block
    ...
    color("Silver") {
        ...
        for(i = [-1, 0, 1]) {
            ...
            translate([pcb_l - pin_offset_x, pcb_w/2 + (i * conn_pitch), -pin_protrusion]) {
                cylinder(d1=0, d2=0.64, h=pin_tip_h, $fn=16);
            }
            // Main pin bodies extending up inside the shroud cavity
            translate([pcb_l - pin_offset_x, pcb_w/2 + (i * conn_pitch), -pin_protrusion + pin_tip_h]) {
...
module safe_line_marker() {
    color("Black") {
        translate([safe_line_x - 0.75, 1, pcb_t]) {
...
module exploded_system() {
    ...
    translate([0, 0, z_pcb_seat + gap]) pcb_assembly();

    translate([0, 0, outer_h + gap * 2]) color("LightBlue", 0.75) lid();
}
